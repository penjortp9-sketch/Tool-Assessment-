...
// WITH NAME-ONLY AUTHENTICATION & ENHANCED EXCEL EXPORT
...
// ============================================
// USERNAME SYSTEM - PERSISTENT STORAGE (UPDATED)
// ============================================
...
// ============================================
// NAME-ONLY AUTHENTICATION SYSTEM
// ============================================
...
function startSession() {
    ...
    if (!userName) {
        ...
        addShakeAnimation(document.getElementById('user-name'));
        return;
    }
    
    currentUserName = userName;
    
    // Save username to master list
    ...
    updateDashboardUserInfo(userName);
    
    document.getElementById("name-screen").classList.add("hidden");
    document.getElementById("main-screen").classList.remove("hidden");
    
    document.getElementById("greeting-name").textContent = userName.split(' ')[0];
...
function updateDashboardUserInfo(userName) {
...
function savePersistentData() {
    ...
    localStorage.setItem(`chimla_persistent_${currentUserName}`, JSON.stringify(persistentData));
}

function loadPersistentData() {
    if (!currentUserName) return false;
    
    const savedData = localStorage.getItem(`chimla_persistent_${currentUserName}`);
...
function clearPersistentData() {
    if (!currentUserName) return;
    localStorage.removeItem(`chimla_persistent_${currentUserName}`);
}
...
function autoGenerateBlocksManual() {
    ...
    for (let idx in blockTimerState) {
        if (blockTimerState[idx] && blockTimerState[idx].animationFrameId) {
            cancelAnimationFrame(blockTimerState[idx].animationFrameId);
        }
    }
    blockTimerState = {};
...
function checkAndTriggerAutoSave() {
    ...
    if (allCompleted && allRated) {
        ...
        const lastAutoSaveKey = `chimla_autosave_${currentUserName}_${today}`;
...
function performAutoSave() {
    ...
    history.push(entry);
    localStorage.setItem(`chimla_history_${currentUserName}`, JSON.stringify(history));
...
function getHistory() { 
    if (!currentUserName) return [];
    const stored = localStorage.getItem(`chimla_history_${currentUserName}`); 
    if (!stored) return [];
...
function clearAllHistory() {
    if (confirm('⚠️ Are you sure? This will clear history from the app.')) {
        if (currentUserName) localStorage.setItem(`chimla_history_${currentUserName}`, JSON.stringify([]));
        filteredHistoryData = [];
...
function exportUserData() {
    if (!currentUserName) { showAlert('Please enter your name first', 'error'); return; }
    const userData = { userName: currentUserName, history: getHistory(), settings: { theme: localStorage.getItem('theme'), dailyReminder: localStorage.getItem('dailyReminderEnabled'), reminderTime: localStorage.getItem('reminderTime') }, exportDate: new Date().toISOString() };
    const dataStr = JSON.stringify(userData, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `chimla_backup_${currentUserName}_${new Date().toISOString().slice(0,10)}.json`;
    link.click();
    ...
    reader.onload = function(e) {
        try {
            ...
            if (importedData.history && Array.isArray(importedData.history)) {
                ...
                localStorage.setItem(`chimla_history_${currentUserName}`, JSON.stringify(uniqueHistory));
...
function downloadHistoryCSV() {
    ...
    link.setAttribute('download', `Chimla_Tabdew_Export_${currentUserName ? currentUserName : 'User'}_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.csv`);
    link.click();
    URL.revokeObjectURL(url);
...
// ============================================
// BLOCK TIMER SYSTEM - OPTIMIZED VERSION (UPDATED)
// ============================================
...
function startBlockTimer(blockIdx) {
    ...
    function tick(now) {
        ...
        if (delta >= 1000) {
            ...
            state.remainingSeconds -= secondsToSubtract;
...
function buildTimerHTML(blockIdx) {
    ...
    const isComplete = state.remainingSeconds <= 0;
    const displayClass = isComplete ? 'timer-done' : '';
    
    return `<div class="block-timer-section">
        <span class="block-timer-label">⏱️ Timer</span>
        <span class="block-timer-display ${displayClass}" id="timer-display-${blockIdx}">${display}</span>
